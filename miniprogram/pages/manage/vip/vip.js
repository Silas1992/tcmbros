...
Page({
  ...
  getAllUser:function(e){
    ...
    wx.cloud.callFunction({
      ...
      success:function(res){
        ...
        let list = that.data.userInfo;
        list = list.concat(userInfo);
        that.setData({
          next: false,
          userInfo: list,
        })
  ...
  formSubmit:function(evt){
    ...
    var price = evt.detail.value.name;
    var _id = evt.detail.target.dataset._id
    var openid = evt.detail.target.dataset.openid
    var name = evt.detail.target.dataset.name
    var tel = evt.detail.target.dataset.tel
    var that = this
    //查询用户信息
    wx.cloud.callFunction({
      name:'getMemberBy',
      ...
      success:function(res){
        var memberInfo = res.result.data[0]
        var newBalance = parseInt(memberInfo.balance)-1+parseInt(price)+1
        that.updateBalance(_id,newBalance,openid,name,tel,price)
      },
      fail(res){
  ...
  //修改用户的余额
  updateBalance:function(_id,balance,openid,name,tel,price){
    var myDate = new Date()
    ...
    wx.cloud.callFunction({
      ...
      success:(res)=>{
        //生成自定义充值记录
        console.log('自定义充值来了')
        this.consumRecharge(openid,name,tel,price)
      },
      fail:console.error
    })
  },
  consumRecharge(openid,name,tel,price){
    let sub_id = this.data.memberInfo.sub_id
    wx.cloud.callFunction({
      name:'addGoodRecord',
      data:{
        memberOpenId:openid,
      ...
      success:(res)=>{
        wx.hideLoading({
          complete: (res) => {
            ...
            console.log("----",openid,name,tel,sub_id,price,res)
            //生成自定义充值记录
            this.onShow()
          },
        })
